...
begin 
 ...
 -- if check_Antivirus='0' then set checkresult=concat(checkresult,'杀毒软件',';'); end if;
  if check_AccountNormal='' then set checkresult=concat(checkresult,'用户账号',';'); end if;
  ...
  -- if check_DataBaseLog='0' then set checkresult=concat(checkresult,'数据库日志',';'); end if;
  if check_HardDriverUsage='' then set checkresult=concat(checkresult,'磁盘空间情况',';'); end if;
